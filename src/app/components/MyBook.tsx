...
export default function MyBook() {
  ...
  const fetchBooks = (pageNumber: number) => {
    ...
    fetch(`${BASE_URL}/api/mybook?page=${pageNumber}&limit=10&${query}`, {
      headers: {
  ...
  return (
    <>
      ...
      <div className="grid lg-grid-cols-5 md-grid-cols-5 sm-grid-cols-1 gap-4 m-10">
        {books.map((book: any, index: number) => (
          <CardMyBook key={book.id} book={book} />
        ))}
        <div ref={lastBookRef} />
      </div>
    </>
